...
class SwingDetectionWebSocketService: NSObject, ObservableObject {
    ...
    @Published var swingsDetected = 0
    ...
    func connect() {
        ...
        let wsURL = Config.serverBaseURL
    ...
    func sendFrame(_ image: UIImage, timestamp: TimeInterval) async throws {
        ...
        do {
            ...
            case "evaluated":
                if let swingDetected = response.swingDetected,
                   ...
                   swingDetected {
                    swingsDetected = response.totalSwings ?? (swingsDetected + 1)
                    print("🏌️ Swing \(swingsDetected) detected with confidence: \(confidence)")
                    onSwingDetected?(confidence)
                }
                
            case "cooldown":
                isInCooldown = true
                cooldownRemaining = TimeInterval(response.cooldownRemaining ?? 0)
                if let totalSwings = response.totalSwings {
                    swingsDetected = totalSwings
                }
